...
struct JSONTreeView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        // Use inline HTML with embedded json-viewer JS
        webView.loadHTMLString(HTMLTemplate.jsonViewer, baseURL: nil)
...
enum HTMLTemplate {
    /// JSON Viewer with embedded json-viewer library for collapsible tree view
    ...
    <script>
        window.addEventListener('DOMContentLoaded', function() {
            if (customElements.get('json-viewer')) {
                ...
                document.getElementById('error').textContent = 'Error: json-viewer not loaded';
            }
        });
        ...
        window.renderJson = function(jsonString) {
            try {
                ...
                if (viewer && customElements.get('json-viewer')) {
                    viewer.data = JSON.parse(jsonString);
                } else {
                    document.getElementById('loading').style.display = 'none';
                    document.getElementById('error').style.display = 'block';
                    document.getElementById('error').textContent = 'Error: json-viewer not available';
                }
            } catch(e) {
                document.getElementById('loading').style.display = 'none';
                document.getElementById('error').style.display = 'block';
                document.getElementById('error').textContent = 'JSON Error: ' + e.message;
            }
        };
    </script>
    </body>
