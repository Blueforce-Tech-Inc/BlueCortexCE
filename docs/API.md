...
| 2026-05-07 | 0.1.0-beta+45 | ZH API doc: fixed incomplete beta+44 fix — `### Get Observations by IDs` and `### Search by File` subsection headers were added but their child `#### POST /api/observations/batch` and `#### GET /api/search/by-file` remained at `####` level, creating empty-parent-nesting; removed both empty `###` parent headers and promoted child endpoints to `###` level (URL shown as code block), fully matching EN heading structure |
