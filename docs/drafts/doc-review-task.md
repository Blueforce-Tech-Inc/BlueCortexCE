...
- **2026-04-01 20:10**: 架构文档 (ARCHITECTURE.md + ARCHITECTURE-zh-CN.md + backend/README.md 新一轮) — 无修复。验证项：13 个 Controller 源码一致（13 .java files, 67 @Mapping annotations, /api/concepts commented out）；29 个 Service 列表完整（EN/ZH/backend README 三版一致，含 XmlParser/util + ClaudeMemMcpTools/mcp 分类标注）；14 个 Flyway 迁移文件一致（V1-V16, V9/V10 跳过）；Schema SQL 准确（mem_sessions 含 V4 context caching 列, mem_observations 含 V11 8 列, mem_user_prompts 含 V5 project_path, mem_pending_messages 含 V6 tool_input_hash + V1 started_processing_at_epoch）；Spring Boot 3.3.13 正确（pom.xml 确认）；环境变量表 14 个变量准确（application.yml / application-dev.yml 确认）；MCP 传输协议 SSE/Streamable 准确；proxy 目录列表准确（含 java/proxy/test-full-flow.mjs）；中英文版本跨链接完整。
- **Next direction**: API 文档 (新一轮轮换)
- **2026-04-01 21:07**: API 文档 (API.md + API-zh-CN.md 新一轮) — 修复 1 问题：英文 PATCH /api/memory/observations 章节严重不完整——缺少路径参数说明、成功响应示例（`status:updated`）和错误响应（中文版已全部包含），已补充完整文档；英文 DELETE /api/memory/observations 补充路径参数说明。commit 5356342。其他验证项：13 个 Controller 源码一致（67 个 @Mapping 注解，/api/concepts 已注释掉）；66 个端点覆盖完整（中英文一致）；Search 策略值正确（hybrid/tsvector/filter/recent/none）；Get Settings 响应格式准确（20 个 CLAUDE_MEM_* 字段）；ObservationCreateRequest 字段名与源码一致；Ingestion 请求格式准确；Changelog 同步。Next direction: SDK README (新一轮轮换)
