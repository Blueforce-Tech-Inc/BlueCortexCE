...
- **2026-04-02 09:08**: SDK README (Go/Python/JS 新一轮) — 修复 1 问题：Go SDK 测试数 268→270（`go test -v ./...` 实际 270 PASS，主包 216 + dto 54，原 268 为过时计数），中英文两版同步更新。commit d3c5a57。其他验证项：25 个 API 方法数（三 SDK 源码逐一确认：Go interface 27 methods 含 Close/String，排除后 25；JS 25 async + close = 26 methods in table；Python 25 API methods）；Go 270 tests（`grep "PASS"` 确认）；JS 212 tests（vitest 确认）；Python 350 tests（pytest --co 确认）；23 个唯一 HTTP 端点路径与 Controller 源码一致（triggerRefinement 使用 query param `?project=...`，getLatestExtraction/getExtractionHistory 使用 `/api/extraction/{templateName}/latest` 带 projectPath query param，三 SDK 实现一致）；中英文版本结构/内容/跨链接一致（6 个 README cross-link 完整）；Features 章节三 SDK 双语一致（含测试计数）；Wire 格式章节三 SDK 双语一致；依赖声明准确（Go 1.22 / Python >=3.10 requests>=2.28 / JS Node>=18）；设计文档交叉链接存在（go-sdk-design.md / js-sdk-design.md / python-sdk-design.md）；Go 子包 README（eino/langchaingo/genkit）结构完整。
- **Next direction**: 设计文档 (新一轮轮换)
