...
1. **STREAMABLE 协议在 Spring AI 1.1.2 下已验证完全可用！** (2026-03-19 实测)
2. **SSE 仍然可用，作为兼容备选协议**
3. **`protocol: STATELESS` 在本版本仍有 404 问题，不推荐使用**
4. **默认协议已切换为 STREAMABLE**，符合 MCP 官方规范演进方向

### 推荐结论

- **默认**: STREAMABLE 协议作为默认配置（已验证可用，2026-03-19）
- **短期**: Streamable HTTP 作为主要协议，SSE 作为兼容备选
- **中期**: 持续跟进 Spring AI 更新，完善会话管理和可观测性
