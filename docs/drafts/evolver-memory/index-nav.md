...
| [127](./127-opencode-adapter-v180-deep-dive.md) | `opencode.js` v1.80.0 新平台适配器深度（234L纯JS + 224L test / 第5个 evolver 平台（继 Claude Code/Codex/Cursor/Kiro）/ 插件系统 `~/.opencode/plugins/evolver.js` 集成 / `session.idle` 触发 session-end（区别于其他平台的 `session.end`）/ `spawnSync` 同步执行+3种超时（3000/2000/8000ms）/ `session.created` 从嵌套 `event.properties.info.id` 提取 session_id / tool.execute.after 仅处理 write/edit 工具 / `isEvolverManagedPluginFile` 防误删用户插件 / 原子写入 tmp+rename / AGENTS.md marker幂等追加 / CE P3 idle-trigger session-end模式+P3幂等安装卸载） |
| [128](./128-v1790-v180-binary-build-and-bootstrap-deep-dive.md) | Binary Build Pipeline + Bootstrap Fix — v1.79.0/v1.79.1 深度（388L `build_binaries.js` + 126L `index.js` delta / 三阶段管线 bun bundle→javascript-obfuscator→bun compile --compile --minify 产5平台独立二进制 / selfDefending必须关闭/renameGlobals必须关闭 / SHA256SUMS.txt / CE P3参考；`index.js` bootstrap修复 Issue #460+526（ATP看不到A2A_SECRET/dotenv加载太晚+getRepoRoot缓存导致EVOLVER_REPO_ROOT静默失效 / 三步加载cwd→repo→root / dotenv不覆盖已存在key / EVOLVER_QUIET_PARENT_GIT=1防误导 / CE P2环境变量时序验证）；spawnReplacementProcess Windows Fix #528（in-process respawn改为可选项 / withPlatform()跨平台测试 / CE P3参考） |
