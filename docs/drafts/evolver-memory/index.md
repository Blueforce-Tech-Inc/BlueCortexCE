...
| **ATP Execute + AutoDeliver + MemoryGraphAdapter + SelfRepair**（atpExecute 四阶段结算管线（read→publish→complete→deliver）+ HMAC签名+幂等asset_id / autoDeliver polling+ledger防重+terminal error负数标记 / memoryGraphAdapter Local/Remote双Provider+本地优先写+远程增强fallback / self_repair 四步git自愈（rebase/merge abort + stale lock age判断 + hard reset gate） / CE P0 async task防漏+P0 MemoryGraphProvider抽象+P1 HMAC签名+P1 terminal error分类） | [`81`](./81-atp-execute-autodeliver-memorygraph-adapter-selfrepair.md) |
