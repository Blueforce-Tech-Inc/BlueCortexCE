...
**最后更新**：2026-05-04（**`83`** ATP 商家端子系统（pickup bridge+Ledger冷却/autoBuyer日预算+冷启动半额+dedup/questionComposer capability→自然语言template/Consumer+Merchant Agent/CLI opt-in）；**`82`** Solidify PRM + Epigenetic + Gene Learning Adaptation（8相评分/表观遗传90天衰减/adaptGeneFromLearning/FailedCapsule rollback/Anti-pattern Hub/Narrative Memory/ValidationReport/7种Strategy/CE P0多维评分）；**`81`** ATP Execute+AutoDeliver+MemoryGraphAdapter+SelfRepair（atpExecute四阶段+HMAC幂等/autoDeliver polling+ledger/Local+Remote双Provider+本地优先写+self_repair四步git自愈）；**`80` v1.78.5 版本增量同步**（Claude Code `tool_input.*` signal-detect 修复 cbc4870，#522：`ti.content`/`ti.new_string`/`ti.file_content` 优先读嵌套payload，fallback 兼容旧格式；不影响 Cursor/Codex/Kiro）；**`79c`** Hook 适配系统深度（hookAdapter 4平台检测+atomic JSON merge+3 hook scripts复制 / signal-detect 7类signal+1.5s timeout+多平台兼容 / session-start 历史记忆注入+去重TTL 30min / session-end git diff+Hub API双路径）；**`79b`** 运维基础设施深度（health_check 4级严重度 / trigger WAKE文件IPC / skills_monitor 自愈 / cleanup 两阶段双保险 / lifecycle graceful shutdown）；**`79`** 基础设施模块深度（paths parent git安全检查 / assetCallLog Append-only / contentHash SHA-256 / deviceId 7层fallback / innovation 技能缺口驱动）；**`78`** Proxy 子系统（MailboxStore 原子写 / OutboundSync MAX_BATCH=50+10次重试 / InboundSync Cursor分页 / CE P0原子写+P1分层路由））  
...
| **ATP + Adapters 系统**（Hub Client 275行（placeOrder/submitDelivery/verify/settle/dispute）/ hookAdapter 207行 4平台（Cursor/ClaudeCode/Codex/Kiro） / 商家模板 118行 / 消费者模板 157行 / autoBuyer ~200行 / CE P1/P2/P3） | [`75`](./75-atp-agent-transaction-protocol-and-adapters.md)；**商家端子系统详见 [83]** |
| **ATP Execute + AutoDeliver + MemoryGraphAdapter + SelfRepair**（atpExecute 四阶段（read→publish→complete→deliver）+ HMAC+幂等asset_id / autoDeliver polling+ledger防重 / memoryGraphAdapter Local/Remote双Provider+本地优先写 / self_repair 四步git自愈 / CE P0 async task防漏+P0 Provider抽象+P1 HMAC+P1 terminal error分类） | [`81`](./81-atp-execute-autodeliver-memorygraph-adapter-selfrepair.md) |
| **Solidify PRM + Epigenetic Marks + Gene Learning Adaptation**（`computeProcessScores` 8相评分（signal×0.05+selection×0.10+mutation×0.05+blast×0.15+constraint×0.25+validation×0.25+protocol×0.10+canary×0.05）/ 表观遗传标记90天衰减+环境三元组（platform/arch/node_version）/ `adaptGeneFromLearning` signals_match扩充+anti_patterns积累 / FailedCapsule rollback前diff捕获 / Anti-pattern Hub opt-in发布 / Narrative Memory 30条/12KB裁剪 / ValidationReport标准化+环境指纹 / 7种Strategy预设+cycle自动检测 / CE P0多维质量评分+P0 ExtractionReport+P1 FailedCapture+P1表观遗传+P2反模式发布） | [`82`](./82-solidify-prm-process-scoring-and-epigenetic-marks.md) |
| **ATP 商家端子系统**（pickup bridge+Ledger / autoBuyer 日预算+冷启动+dedup / questionComposer template / Consumer+Merchant Agent 生命周期） | [`83`](./83-atp-merchant-side-task-pickup-autobuyer-and-agent-templates.md) |
...
| **IdleScheduler OMLS 自适应调度**（4级强度 / 跨平台空闲检测 / `sleep_multiplier` 休眠控制 / `should_distill`+`should_reflect`+`should_deep_evolve` 三联动 / CE cron 任务强度分级提案 P0） | [77](./77-idleScheduler-contentHash-OMLS-adaptive-memory-scheduling.md) |
| **Content-Addressable Asset**（`canonicalize` 规范化 JSON + SHA-256 内容寻址 / 篡改检测 / Schema Version / CE ObservationEntity 去重 P2） | [77](./77-idleScheduler-contentHash-OMLS-adaptive-memory-scheduling.md) |
| **v1.78 新增 Proxy 子系统**（~2392 行：EvoMapProxy 中间代理 / JSONL+Mem MailboxStore（prototype pollution 防护）/ Outbound+Inbound SyncEngine / LifecycleManager(hello/hb/reauth) / REST Routes(/mailbox /asset /task /session /dm) / SessionHandler P2P 协作 / hookAdapter 跨平台检测 / CE P0（原子写+Cursor分页+批量重试）/ P1（分层API路由）/ P2（Proxy中间层思想）） | [78](./78-v178-proxy-subsystem-architecture.md) |
