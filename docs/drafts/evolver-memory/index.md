...
**最后更新**：2026-04-19（`15` §2.1 Hook Worker 基址）
...
| [12](./12-bluecortex-api-memory-surface.md) | **HTTP**、§1.1 **`semantic` 契约**、§2 **数据平面**、**§3** 调用方 |
...
| [12](./12-bluecortex-api-memory-surface.md) | **§1** 读出 · **§1.1** `semantic` 契约 · **§2** 写入（SQLite+Chroma ∥ Postgres+pgvector）· 调用方 |
