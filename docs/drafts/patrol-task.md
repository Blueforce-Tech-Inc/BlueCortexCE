...
- SDK/Demo 问题当场修复，Backend 问题记录到 findings.md（由低频 cron 集中修复）
...
3. **代码审查**（本次轮到的方向，~5分钟）：
   ...
   - Backend 问题 → **记录到 `docs/drafts/backend-review-findings.md`**（由低频 cron 任务集中修复）
4. **git commit**（如有修复）
...
| 问题类型 | SDK/Demo（直接修复） | Backend（记录到 findings.md） |
|----------|---------------------|------------------------------|
| Bug | 修复 + 编译/测试验证 + commit | 记录，待低频 cron 集中修复 |
| 文档缺失 | 更新 + commit | 记录 |
| 测试缺失 | 补测试 + 跑测试 + commit | 记录 |
| 架构改进 | 记录到 docs/drafts/ | 记录 |

## 已完成实施基准

- Java SDK: 24 个 API 方法
- Go SDK: 26 个 API 方法 + 8 DTO + 3 集成层 (277 tests)
- Python SDK: 25 个 API 方法 + 15 DTO + ObservationUpdate + Flask Demo (343 tests)
- JS/TS SDK: 25 个 API 方法 + CJS/ESM/DTS 输出 (163 tests)
