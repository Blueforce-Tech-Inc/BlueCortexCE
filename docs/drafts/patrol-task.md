...
- **Backend 问题记录到 findings.md** — 由专门的低频 cron 任务（或手动触发）集中修复
...
### 6. Backend 代码审查 ⚠️ 仅记录不修复（由低频 cron 集中修复）
...
### 7. 文档审查 ⚠️ 发现问题必须当场修复或记录
- Phase 3 设计文档 (`docs/drafts/phase-3-design.md`) 与实现一致性
- API.md / API-zh-CN.md 端点覆盖完整性
- README、架构文档的准确性
- 巡检任务自身文档的完整性
- **⚠️ 文档问题同样适用"发现必须处理"规则——要么当场修复，要么记录到 findings.md**

## 发现问题时的行动（每个问题必须有落点）

| 问题类型 | SDK/Demo（直接修复） | Backend（记录到 findings.md 或直接修复） | 文档（直接修复 或 记录） |
|----------|---------------------|------------------------------|----------------------|
| Bug | 修复 + 编译/测试验证 + commit | **必须记录到 findings.md**（或小问题当场修复） | — |
| 文档缺失/过时 | 更新 + commit | **必须记录到 findings.md** | **必须当场修复**（或记录到 findings.md） |
| 测试缺失 | 补测试 + 跑测试 + commit | **必须记录到 findings.md** | — |
| 架构改进 | 记录到 docs/drafts/ | **必须记录到 docs/drafts/** | — |
| 设计文档过时 | 修复 + commit | **必须记录或直接修复** | **必须当场修复** |

**⚠️ 所有审查类型（代码 + 文档）适用同一条规则：发消息汇报 ≠ 处理。每个问题必须落到上述表格对应的行动里。**
...
- JS/TS SDK: 25 个 API 方法 + CJS/ESM/DTS 输出 (198 tests)
