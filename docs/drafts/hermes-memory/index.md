...
| **105** | [`60-evolution/105-shell-hooks-extensibility-architecture.md`](60-evolution/105-shell-hooks-extensibility-architecture.md) | **Shell Hooks 插件扩展架构**：836 行；17 种 VALID_HOOKS 事件（pre/post_tool_call · pre/post_llm_call · pre_gateway_dispatch 等）；JSON wire protocol（stdin event + stdout action/context）；`shell=False` 防 shell 注入；allowlist + TTY 同意书 + atomic_replace 写入；与 Python 插件共存于 PluginManager；CE 落地：P1 MCP 层自定义 pre-tool guard + P1 同意书安全模型 + P1 超时控制 |
