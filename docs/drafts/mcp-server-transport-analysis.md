...
> 一句话：`SSE` 是**兼容性最好、无 session 管理问题**的模式；`STREAMABLE` 是推荐标准但需要客户端正确实现 session 管理。

### 5.4 最终结论 ✅ (2026-03-19)

**最终决定**：`protocol: SSE` 作为默认协议！

**理由**:
- SSE 无 session 管理要求，兼容所有 MCP 客户端
- STREAMABLE 要求客户端正确实现 `Mcp-Session-Id` header
- STATELESS 有 Spring AI 1.1.2 Bug (返回 404)
- 应用层足够健壮（project-based 隔离）

**验证命令** (SSE 模式):
```bash
curl -X POST http://127.0.0.1:37777/sse \
...
2. SSE 兼容性最好，无需会话管理
3. STREAMABLE 已验证通过 (14/14)，但作为备选
