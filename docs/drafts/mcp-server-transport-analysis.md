...
| Issue | 描述 | 状态 |
|-------|------|------|
| [#3947](https://github.com/spring-projects/spring-ai/issues/3947) | 多实例部署下 SSE 连接失败（会话状态不同步） | 未解决（2026-01） |
...
| 断线恢复 | 无（需重新握手） | 支持 `Last-Event-ID` 恢复 |
| 多实例部署 | 高风险 | 原生支持 |
| MCP 规范合规 | 已废弃 | 当前标准 |
...
**根因**：`STATELESS` 是一个合法的枚举值，配置名称本身没有错误。404 的真实原因更可能是 **Spring AI 1.1.2 中 `STATELESS` 模式的 AutoConfiguration 在 WebMVC 下存在 Bug**，导致 `/mcp` 端点未被正确注册。这与 Spring AI 社区中 MCP AutoConfiguration 重构（PR #4179）的背景一致——该重构正是为了修复 Streamable HTTP 支持问题。

> 注：`STATELESS` 不支持向 MCP 客户端发送消息请求（如 elicitation、sampling、ping）。`STREAMABLE` 支持完整能力。

### 5.3 待验证

需要验证 `protocol: STREAMABLE` 是否能在当前版本（Spring AI 1.1.2）下正确注册 `/mcp` 端点。如果仍然 404，可尝试升级到 Spring AI 1.1.3+（PR #4179 合并后的版本）。
...
|--------|---------------------|------|
| Claude Code（claude CLI） | ✅ 支持（v1.0.27+） | `claude mcp add --transport http <url>` |
| Cursor IDE | 需验证 | 查看 Cursor MCP 文档 |
| Spring AI MCP Client | ✅ 支持 | `spring-ai-starter-mcp-client` |

> Claude Code 从 v1.0.27 起支持 `--transport http` 选项连接 Streamable HTTP MCP 服务端。  
> 来源：[anthropics/claude-code#1387](https://github.com/anthropics/claude-code/issues/1387)  
> 注意：Claude Code Streamable HTTP 客户端存在已知 Bug（[#5960](https://github.com/anthropics/claude-code/issues/5960)）：长时间流式工具调用只显示第一个 chunk，后续输出不实时。
...
- 关注 MCP 2026 路线图：官方计划进一步消除会话依赖，使协议完全无状态
