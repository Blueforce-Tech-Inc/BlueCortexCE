...
### 第四十五轮（2026-03-25 18:01）— ViewerController 全面异常处理 + 类型安全修复
- [x] ViewerController.search() 添加 try-catch（之前 searchService.search() 抛异常时返回原始 500 栈追踪，与所有其他控制器不一致）
- [x] ViewerController.batchGetObservations() 修复不安全类型转换：`(List<String>) request.get("ids")` → instanceof 类型检查 + stream 过滤（之前发送 `{"ids":"not-a-list"}` 导致 ClassCastException → 原始 500）
- [x] ViewerController.batchGetObservations() 新增空 UUID 列表校验（所有 UUID 都无效时返回 400 而非空结果）
- [x] ViewerController.batchGetObservations() 添加 try-catch 包裹数据库查询
- [x] ViewerController.batchGetSessions() 同样修复不安全类型转换（`@SuppressWarnings("unchecked")` 移除，改用 instanceof 安全检查）
- [x] ViewerController.saveSettings() 错误响应从 HTTP 200 改为 HTTP 500（与 round 40-41 修复的其他控制器一致）
- [x] ViewerController.getTimeline() try-catch 包裹范围从 stream 处理扩展到 repository 调用（之前 findTimelineByDate 抛异常时仍返回原始 500）
- [x] ViewerController.searchByFile() 添加 try-catch（之前 findByFolderPath 抛异常时返回原始 500）
- [x] Java Backend BUILD SUCCESS、Go SDK 88 测试通过、go vet 干净、回归测试 46/46 PASS
