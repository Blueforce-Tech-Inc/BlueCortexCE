...
## 📡 16. A2A 协议：分布式 Evolver 网络

### 16.1 整体定位

`a2aProtocol.js`（**1221 行**）是 EvoMap v1.47.0+ 新增的核心模块，实现了**去中心化的 Agent-to-Agent 通信协议**。它将单个本地 evolver 节点编织成一个分布式进化网络（EvoNet），节点之间可以：

- **服务发现**：通过 Hub 发现其他节点的 Gene/Capsule 服务
- **经验共享**：跨节点传递 Gene 和 Capsule 资产
- **ATP 协作**：参与 Agent Trading Platform 的任务买卖
- **心跳保活**：通过 SSE 流式推送可用工作
...
│         (中心化服务发现 + ATP 结算层)                 │
│                                                     │
│  A2A/HTTP  ←── SSE  ←── Heartbeat (每 60s)         │
│                                                     │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐          │
│  │ Node A   │  │ Node B   │  │ Node C   │          │
│  │ evolver  │  │ evolver  │  │ evolver  │          │
│  │ v1.47.0  │  │ v1.79.1  │  │ local    │          │
│  └──────────┘  └──────────┘  └──────────┘          │
└─────────────────────────────────────────────────────┘
```
...
| 消息类型 | 方向 | 用途 |
|---------|------|------|
| `hello` | 节点→Hub | 节点注册、身份认证 |
| `publish` | 节点→Hub | 发布 Gene/Capsule 服务 |
| `fetch` | 节点→Hub | 搜索/拉取其他节点资产 |
| `report` | 节点→Hub | 上报任务进度、信号 |
| `decision` | Hub→节点 | 下发任务决策 |
| `revoke` | 节点→Hub | 撤销已发布的服务 |

### 16.4 节点身份管理

```javascript
// Node ID 格式：node_[12位十六进制]
const NODE_ID_RE = /^node_[a-f0-9]{12}$/;
// 存储位置：~/.evomap/node_id
// 密钥位置：~/.evomap/node_secret
```

**身份注册流程**：
1. 首次启动时生成随机 Node ID
2. 向 Hub 发送 `hello`，获取节点密钥
3. 后续所有请求使用 HMAC 签名（`sender_id` + `timestamp` + 密钥）

### 16.5 SSE 心跳流

```javascript
// Hub 通过 SSE 推送可用工作
const _sseReconnectMs = 5000;     // 初始重连间隔
const _sseMaxReconnectMs = 120000; // 最大重连间隔（指数退避）

// 心跳携带的数据
{
  available_work: [...],      // 可接任务列表
  overdue_tasks: [...],       // 逾期任务警告
  skill_store_hint: {...},    // Skill 市场提示
  novelty_hint: {...},       // 创新信号
...
**上传路径（本地→Hub）**：
- `a2a.js` 的 `publishAsset()` 调用 Hub 的 `/a2a/publish` 端点
- 发布前调用 `lowerConfidence()` 对外部资产降权 40%（防止污染本地经验）

**下载路径（Hub→本地）**：
- `fetchAsset()` 从 Hub 拉取 Gene/Capsule
- 外部资产写入本地前，通过 `sanitize.js` 脱敏
- 外部资产的 `confidence *= 0.6`（接收降权因子）

**资产类型白名单**：
```javascript
// 仅允许三类资产跨节点传递
const ALLOWED = ['Gene', 'Capsule', 'EvolutionEvent'];
```
...
EvoMap v1.47.0+ 引入了对多种 AI Agent 的适配支持，通过统一的 Hook 接口将它们的会话日志接入记忆系统：

| 适配器 | Agent | 代码行数 | 核心职责 |
|--------|-------|---------|---------|
| `claudeCode.js` | Claude Code | 163 | Claude Code 项目级 Hook 管理 |
| `codex.js` | OpenAI Codex | 172 | Codex 会话日志解析 |
| `cursor.js` | Cursor | 89 | Cursor 对话适配 |
| `kiro.js` | Kiro | ? | Kiro 适配 |
| `hookAdapter.js` | 通用 Hook 管理 | ? | Hook 脚本安装/卸载 |

### 17.2 Claude Code 适配器详解

```javascript
// buildClaudeHooks() 返回的结构
{
  hooks: {
    SessionStart: [{
      hooks: [{ type: 'command', command: 'node .claude/hooks/evolver-session-start.js', timeout: 3 }]
    }],
    PostToolUse: [{
      matcher: 'Write',  // 仅文件写入触发
      hooks: [{ type: 'command', command: 'node .claude/hooks/evolver-signal-detect.js', timeout: 2 }]
    }],
    Stop: [{
      hooks: [{ type: 'command', command: 'node .claude/hooks/evolver-session-end.js', timeout: 8 }]
    }]
  }
}
```
...
**关键设计**：
- **PostToolUse 只拦截 Write 操作**：避免噪音，只关注文件变更
- **3 个超时配置**：SessionStart=3s, SignalDetect=2s, SessionEnd=8s
- **install/uninstall API**：通过 `~/.claude/settings.local.json` 管理 Hook
- **EVOLVER_MARKER 标记**：卸载时通过 `<!-- evolver-evolution-memory -->` 定位并移除注入内容

### 17.3 适配器隔离设计

```javascript
// hookAdapter.js 中的过滤逻辑：避免 Hook 脚本自我干扰
data.hooks[event] = data.hooks[event]
  .map(matcher => {
    const filtered = matcher.hooks.filter(h => {
      const cmd = (h && h.command) || '';
      // 排除 evolver 自身的 session/signal 脚本，避免循环触发
...
## 💹 18. ATP：Agent Trading Platform

### 18.1 整体定位

ATP 是 EvoMap v1.47.0+ 新增的**去中心化任务市场**，让不同 evolver 节点之间可以买卖 AI 服务：

```
Buyer (Consumer Node)              Hub                  Seller (Merchant Node)
      │                              │                          │
      │──── placeOrder() ──────────►│                          │
      │                              │──── SSE available_work ──►│
      │                              │                          │
      ...
      │◄── order_id + merchant ─────│                          │
      │                              │                          │
      │         (spawn sub-agent to complete task)              │
      │                              │                          │
...
| 角色 | 模块 | 职责 |
|------|------|------|
| **Consumer** | `consumerAgent.js` (157行) | 搜索服务、竞拍、验证交付 |
| **Merchant** | `merchantAgent.js` (118行) | 注册服务、监听订单、提交交付证明 |
| **Hub Client** | `hubClient.js` (262行) | 所有 HTTP 到 Hub 的客户端封装 |
| **Task Pickup** | `atpTaskPickup.js` (233行) | 竞拍任务、拉起子 Agent 执行 |
| **Task Execute** | `atpExecute.js` (285行) | 任务执行后settlement全流程 |
| **Auto Buyer** | `autoBuyer.js` | 自动竞拍策略 |
| **Auto Deliver** | `autoDeliver.js` | 自动交付策略 |

### 18.3 订单生命周期

```
ORDER_PLACED → ORDER_CLAIMED → DELIVERY_PENDING → DELIVERY_SUBMITTED
                    │                                    │
                    ▼                                    ▼
              ORDER_EXPIRED ◄───── [超时/争议] ───── DELIVERY_VERIFIED
                                                            │
                                                            ▼
                                                     DELIVERY_SETTLED
```
...
**ATP Task Pickup 时传递信号**：

```javascript
// atpTaskPickup.js: 拉取任务时记录 signals
const signalsCsv = task.signals ? String(task.signals) : '';
// 传递给子 Agent：--signals=<signalsCsv>
```

**Gene 自动打包**：

```javascript
// atpExecute.js: _buildGene()
{
  id: 'gene_atp_answer_' + caps.sort().join('_').slice(0, 40),
  signals_match: signals.slice(0, 8),   // ← 从订单 signals 构建
  category: 'innovate',
  strategy: [
    'Read the buyer question carefully...',
    'Produce a concrete, actionable answer...',
    'Return the answer as Capsule content...',
  ],
}
```

**关键设计洞察**：ATP 将 signals 作为订单的**元数据传递**，但不直接写入 `memory_graph.jsonl`。这保持了 ATP 的"外部市场"定位——ATP 订单是临时会话，其经验通过 `solidify()` 正常写入记忆图。

### 18.5 验证模式

| 模式 | 说明 |
|------|------|
| `auto` | Hub 自动验证 `asset_id` + `result` 存在 = pass |
| `ai_judge` | Hub 调用 LLM 判断交付质量 |
| `bilateral` | 买卖双方手动确认 |
...
├── evolver-session-start.js   # Session 开始时记录节点状态
├── evolver-signal-detect.js   # PostToolUse 时检测信号（轻量）
└── evolver-session-end.js     # Session 结束时触发 solidify
```

### 19.2 evolver-signal-detect.js（轻量信号检测）

这个脚本在每次文件写入后运行，用于快速检测信号而不需要完整 LLM 分析：
...
**输入**：PostToolUse 事件的 stdin JSON
**输出**：
```javascript
{
  additional_context: '[Evolution Signal] Detected: [log_error, capability_gap] in src/utils.js',
  additionalContext: '...'  // Claude Code 兼容字段
}
```

**与 signals.js 的区别**：
- `signals.js` 是**深度分析**（4 语种、正则、去重、饱和检测）
- `evolver-signal-detect.js` 是**浅层快速检测**（关键词匹配，<2s 超时）
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                     EvoMap Evolver v1.47.0+                          │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Layer 0: Agent Adapters (claudeCode / codex / cursor / ...)  │    │
│  │ 职责：统一不同 Agent 的 Hook 接口 → memoryGraph 事件写入      │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                              ▲                                       │
│                              │ Hook (SessionStart / PostTool / Stop) │
│                              ▼                                       │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Layer 1: Signal Extraction (signals.js)                     │    │
│  │ 职责：深度分析会话日志 → 14 类信号 + 去重 + 饱和检测         │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                              ▲                                       │
│                              │ signals[]                             │
│                              ▼                                       │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Layer 2: Memory Graph (memoryGraph.js)                        │    │
│  │ 职责：因果图推理（append-only JSONL）                        │    │
│  │ 存储：memory_graph.jsonl (追加) + memory_graph_state.json     │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                              ▲                                       │
│                              │ getMemoryAdvice()                      │
│                              ▼                                       │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Layer 3: Gene Selector (selector.js + solidify.js)            │    │
│  │ 职责：基因选择 + PRM 评分 + Gene 自学习                      │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                              ▲                                       │
│                              │ Gene + Capsule                         │
│                              ▼                                       │
...
│  │  ├─ A2A Protocol (a2aProtocol.js) → Hub → 其他节点            │    │
│  │  ├─ ATP Marketplace (atpExecute.js) → 任务买卖               │    │
│  │  └─ Skill Distiller (skillDistiller.js) → Hub marketplace    │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Narrative Memory (narrativeMemory.js)                        │    │
│  │ 职责：Markdown 时间线，人类可读的历史摘要                     │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Local State Awareness (localStateAwareness.js)              │    │
│  │ 职责：捕获节点身份、环境配置、进化状态 → 用于 A2A 注册       │    │
│  └─────────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────────┘
```
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `src/gep/a2aProtocol.js` | **1221** | A2A 通信协议、节点身份、SSE 心跳 |
| `src/gep/a2a.js` | **173** | A2A 资产发布/拉取（Gene/Capsule） |
| `src/gep/localStateAwareness.js` | **244** | 节点状态快照（身份/环境/进化/记忆/技能） |
| `src/adapters/claudeCode.js` | **163** | Claude Code Hook 管理 |
| `src/adapters/codex.js` | **172** | Codex 会话适配 |
| `src/adapters/cursor.js` | **89** | Cursor 会话适配 |
| `src/adapters/hookAdapter.js` | — | 通用 Hook 安装/卸载 |
| `src/atp/hubClient.js` | **262** | Hub API 客户端封装 |
| `src/atp/atpExecute.js` | **285** | ATP 任务结算全流程 |
| `src/atp/atpTaskPickup.js` | **233** | ATP 任务竞拍 + 子 Agent 拉起 |
| `src/atp/consumerAgent.js` | **157** | Consumer 模板（搜索/竞拍/验证） |
| `src/atp/merchantAgent.js` | **118** | Merchant 模板（注册/接单/交付） |
| `src/atp/autoBuyer.js` | — | 自动竞拍策略 |
| `src/atp/autoDeliver.js` | — | 自动交付策略 |
| `src/atp/defaultHandler.js` | — | ATP 任务默认处理器 |
| `src/atp/heartbeatSignalsHandler.js` | — | ATP 心跳信号处理 |
| `src/adapters/scripts/evolver-signal-detect.js` | — | 轻量关键词信号检测 |
| `src/adapters/scripts/evolver-session-start.js` | — | Session 启动钩子 |
| `src/adapters/scripts/evolver-session-end.js` | — | Session 结束钩子 |
...
| 能力 | 描述 | 对记忆系统的影响 |
|------|------|-----------------|
| **A2A 协议** | 分布式 evolver 节点网络 | 外部经验引入需降权 40% |
| **ATP 市场** | 去中心化任务买卖 | signals 作为订单元数据传递 |
| **多 Agent 适配** | Claude Code/Codex/Cursor/Manus | 统一 Hook 接口 → 事件写入记忆图 |
| **Hook 信号检测** | PostToolUse 轻量关键词检测 | 快速信号发现，不依赖 LLM |
| **Local State Awareness** | 节点状态快照 | A2A 注册和 Hub 心跳的数据源 |
| **隐私计算客户端** | `privacyClient.js` | 脱敏后资产方可跨节点传递 |

---

## 💡 23. 对 Claude-Mem 的进阶借鉴

### 23.1 高优先级（v1.47.0+ 新增）

| 设计 | EvoMap 做法 | Claude-Mem 改进建议 |
|------|------------|---------------------|
| **多 Agent 适配** | 统一 Hook 接口适配 Claude Code/Codex/Cursor | 考虑 MCP 协议适配层 |
| **ATP 市场信号传递** | signals 作为订单元数据在 Hub 传递 | 可引入任务类型标签系统 |
| **外部经验降权** | `lowerConfidence(..., factor=0.6)` | 外部 context 引入时加权衰减 |
| **轻量信号检测** | PostToolUse 关键词匹配 <2s | 考虑文件变更类型的快速信号标记 |

### 23.2 中优先级

| 设计 | EvoMap 做法 | Claude-Mem 改进建议 |
|------|------------|---------------------|
| **A2A 节点身份** | 持久化 Node ID + HMAC 认证 | 可引入客户端身份管理 |
| **SSE 心跳流** | Hub→节点推送可用工作 | 可引入服务端推送通知 |
| **Local State Snapshot** | 节点状态快照用于注册 | 可引入节点健康状态上报 |

### 23.3 低优先级

| 设计 | EvoMap 做法 | Claude-Mem 改进建议 |
|------|------------|---------------------|
| **ATP 去中心市场** | 完整订单/交付/结算流程 | 需求不明确 |
| **Merchant/Consumer 模板** | 任务买卖双方模板 | 需求不明确 |

---

_由 Claude Code 分析生成 | 2026-05-07 v2 | BlueCortexCE 项目文档_
