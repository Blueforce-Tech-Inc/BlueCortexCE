...
SESSION_RESP=$(curl -sf -X PATCH "$DEMO_BASE/session/user" \
    -H "Content-Type: application/json" \
    -d "{\"session_id\": \"js-demo-e2e-session\", \"user_id\": \"test-user\"}" 2>/dev/null || echo "FAIL")
...
INGEST_PROMPT_RESP=$(curl -sf -X POST "$DEMO_BASE/ingest/prompt" \
    -H "Content-Type: application/json" \
    -d "{\"project\": \"$PROJECT\", \"session_id\": \"js-demo-e2e-ingest\", \"prompt\": \"test prompt\"}" 2>/dev/null || echo "FAIL")
...
SESSION_END_RESP=$(curl -sf -X POST "$DEMO_BASE/ingest/session-end" \
    -H "Content-Type: application/json" \
    -d "{\"project\": \"$PROJECT\", \"session_id\": \"js-demo-e2e-ingest\"}" 2>/dev/null || echo "FAIL")
