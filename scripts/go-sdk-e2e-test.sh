...
echo ""
echo "Remaining methods (covered in supplementary tests below):"
echo "  → RecordSessionEnd (Test 36: /ingest/session-end)"
echo "  → RecordUserPrompt (Test 35: /ingest/prompt)"
echo "  → TriggerRefinement (Test 30: /refine)"
echo "  → SubmitFeedback (Test 31: /feedback)"
echo "  → UpdateObservation (Test 33: PATCH /observations/{id})"
echo "  → DeleteObservation (Test 34: DELETE /observations/{id})"
echo "  → GetLatestExtraction (Test 28: /extraction/latest)"
echo "  → GetExtractionHistory (Test 29: /extraction/history)"
